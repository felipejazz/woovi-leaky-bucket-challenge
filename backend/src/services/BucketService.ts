...
import {
  ...
  BucketUnexpectedError,
  CreateBucketError,
  BucketRevokeTokenError,
  BucketTokenVerificationError,
} from '../interfaces/Bucket/Errors';
...
export class BucketService {
  static async createBucket(
    user: IDocumentUser
  ): Promise<Result<IDocumentBucket>> {
    ...
    if (!bucketResult) {
      return new Result<IDocumentBucket>({
        success: false,
        data: null,
        error: new BucketUnexpectedError(
          'Errow while ty to get user from bucket'
        ),
      });
    }
    ...
    if (
      ...
      bucketResult.error instanceof BucketNotFoundError
    ) {
      ...
      const bucketToCreate = new BucketModel({
        userName: userAuthDb.userName,
        tokens: [],
      }) as IDocumentBucket;
    ...
    if (!userBucket) {
      return new Result<IDocumentBucket>({
        ...
        data: null,
        error: new BucketUnexpectedError(
          'Error unpacking userBucket after succefully retrieve from db'
        ),
      });
    }
    ...
    try {
      ...
      if (!userBucket) {
        ...
        return new Result<IDocumentBucket>({
          success: false,
          data: null,
          error: new BucketNotFoundError(
            `Bucket not found for user: ${userName}`
          ),
        });
      }
      ...
      if (tokensToAdd > 0) {
        const newTokenCount = Math.min(
          userBucket.tokens.length + tokensToAdd,
          MAX_TOKENS
        );
        const tokensToActuallyAdd = newTokenCount - userBucket.tokens.length;

        for (let i = 0; i < tokensToActuallyAdd; i++) {
          userBucket.tokens.push(
            UserService.generateToken(userBucket.userName)
          );
        }
        ...
        logger.info(
          `Tokens added to bucket for user: ${userName}. New token count: ${userBucket.tokens.length}`
        );
        return new Result<IDocumentBucket>({
          success: true,
      ...
      logger.warn(
        `An error occurred when retrieving bucket for user: ${userName}`
      );
      return new Result<IDocumentBucket>({
        success: false,
        data: null,
  ...
  static async fillBucket({
    ...
    bucket.tokens = [];
    const tokensToAdd =
      MAX_TOKENS - bucket.tokens.length - (initialToken ? 1 : 0);
    ...
    if (!updatedBucket) {
      return new Result<IDocumentBucket>({
        success: false,
        data: null,
        error: new BucketUnexpectedError('Error unpacking updated bucket data'),
      });
    }
    ...
    try {
      ...
      if (!updatedBucket) {
        return new Result<IDocumentBucket>({
          ...
          data: null,
          error: new BucketUnexpectedError(
            `Unexpected error during find and updatefor user: ${bucket.userName}`
          ),
        });
      }
      ...
      logger.error(
        `Error updating bucket for user: ${bucket.userName}. Error: ${error.message}`
      );
      return new Result<IDocumentBucket>({
        success: false,
        data: null,
  ...
  static async addToken({
    ...
    if (!updateBucketResult.success) {
      return new Result<IDocumentBucket>({
        success: false,
        data: null,
        error: updateBucketResult.error,
      });
    }
    ...
    if (!updatedBucket) {
      return new Result<IDocumentBucket>({
        success: false,
        data: null,
        error: new BucketUnexpectedError('Error unpacking updatedBucket data'),
      });
    }
    logger.info(
      `Added new token for user: ${updatedBucket.userName}, tokens count: ${updatedBucket.tokens.length}`
    );
  ...
  static async revokeToken({
    ...
    if (bucket.revokedTokens.includes(token)) {
      ...
      return new Result<IDocumentBucket>({
        success: false,
        data: bucket,
        error: new BucketRevokeTokenError('Token is already revoked'),
      });
    }
    if (!bucket.tokens.includes(token)) {
      ...
      return new Result<IDocumentBucket>({
        success: false,
        data: bucket,
        error: new BucketRevokeTokenError('Token does not belong to user'),
      });
    }
    ...
    if (!bucketUpdateResult) {
      ...
      return new Result<IDocumentBucket>({
        success: false,
        data: bucket,
        error: new BucketRevokeTokenError('Error updating user'),
      });
    }
    ...
    if (!bucketUpdateResult.success) {
      ...
      return new Result<IDocumentBucket>({
        success: false,
        data: bucket,
        error: bucketUpdateResult.error,
      });
    }
  ...
  static async verifyToken({
    ...
    try {
      const decoded = jwt.verify(
        token,
        process.env.SECRET_KEY || 'woovi-challenge-secret'
      ) as IDecodedJWT;
      return new Result<IDecodedJWT>({ success: true, data: decoded });
    } catch (error) {
      if (!(error instanceof jwt.JsonWebTokenError)) {
        return new Result<IDecodedJWT>({
          success: false,
          data: null,
          error: new BucketUnexpectedError(),
        });
      ...
      return new Result<IDecodedJWT>({
        success: false,
        data: null,
        error: new BucketTokenVerificationError(error.message),
      });
    }
  }

  static async checkIfBucketIsEmpty(
    bucket: IDocumentBucket
  ): Promise<Result<boolean>> {
    logger.info(`Ver if bucket token is in user bucket: ${bucket.userName}`);
    if (bucket.tokens.length === 0) {
      logger.warn(
        `Bucket is empty. No valid tokens available for user: ${bucket.userName}`
      );
      return new Result({
        success: false,
        data: false,
        error: new BucketNoValidTokensError(),
      });
    }
  ...
  static async getTokenCount(user: IDocumentUser): Promise<Result<number>> {
    ...
    if (!bucketToCount) {
      return new Result<number>({
        success: false,
        data: null,
        error: new BucketCountError('Bucket object Result is undefined'),
      });
    }
    if (!bucketToCount.tokens) {
      return new Result<number>({
        success: false,
        data: null,
        error: new BucketCountError('Bucket token list object is undefined'),
      });
    }
    logger.info(
      `Getting token count for user: ${user.userName}, tokens count: ${bucketToCount.tokens.length}`
    );
