...
export class PixService {
  ...
  static async makePix({
    ...
    value: number;
    token: string;
  }): Promise<Result<{ token: string | null; count: number | null }>> {
    ...
    if (!bucketResult) {
      ...
      return new Result<{ token: string | null; count: number | null }>({
        success: false,
        error: error,
        data: null,
      });
    ...
    if (!bucketResult.data) {
      const error = new Error(
        'Failed to retrieve bucket data to consume token'
      );
      return new Result<{ token: string | null; count: number | null }>({
        success: false,
        error: error,
    ...
    const job = await this.pixQueue.add(
      'process-pix',
      { userName, bucket, token, key, value },
      {
        attempts: 1,
        backoff: 0,
      }
    ...
    if (jobResult instanceof Error) {
      ...
      return new Result<{ token: string | null; count: number | null }>({
        success: false,
        data: { token: newUserToken, count: tokensLeft },
        error: jobResult,
      });
    }
    newUserToken = token;
  ...
  private static async waitForJobCompletion(
    ...
    return job
      .waitUntilFinished(this.pixQueueEvents)
      .then(() => {
      ...
      .catch((error) => {
        if (!(error instanceof Error)) {
          logger.error(
            'Error is not an instance of Error, wrapping it in a new Error.'
          );
          error = new Error(
  ...
  static isValidPixKey(key: string): Result<boolean> {
    ...
    if (!emailRegex.test(key) && !phoneRegex.test(key)) {
      ...
      return new Result<boolean>({
        success: false,
        data: null,
        error: new InvalidPixKeyError(key),
      });
    }
  ...
  static isValidPixValue(value: number): Result<boolean> {
    if (typeof value !== 'number' || value <= 0) {
      ...
      return new Result<boolean>({
        success: false,
        data: null,
        error: new InvalidPixValueError(value),
      });
    }
    return new Result<boolean>({ success: true, data: true });
  }
  ...
  static createPixError(message: string): PixError {
    ...
    if (message === 'Invalid PIX VALUE. only positives values are allowed.') {
      ...
      message ===
      'Invalid PIX KEY FORMAT. Allowed format: email@example.om or telephone +5511999999999'
    ) {
