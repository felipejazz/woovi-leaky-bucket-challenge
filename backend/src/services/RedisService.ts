...
class RedisService {
  ...
  private constructor() {
    ...
    logger.info(
      `Redis instance created. Total instances: ${RedisService.instanceCount}`
    );
  }
  ...
  static async teardown(): Promise<void> {
    ...
    RedisService.instanceCount--;
    logger.info(
      `Redis instance closed. Remaining instances: ${RedisService.instanceCount}`
    );
  }
  ...
  static getInstance(): RedisService {
    if (!RedisService.instance) {
      throw new Error(
        'RedisService is not initialized. Call initialize() first.'
      );
    }
