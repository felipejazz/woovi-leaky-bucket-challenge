...
import { FailedCreateUserError, UserNotFoundError, UserUnexpectedError } from '../interfaces/User/Errors';
...
export class UserService {
        ...
        try {
            ...
            if (!updatedUser) {
                return new Result<IDocumentUser>({
                    success: false,
                    data: null,
                    error: new UserUnexpectedError(`Unexpected error during find and update for user: ${user.userName}`),
                });
            }
    ...
    static async getUser(userName: string): Promise<Result<IDocumentUser>>{
        try {
            ...
            if (!(error instanceof Error)) {
                logger.warn(`An unexpected error occurred in mongo when retrieving bucket for user: ${userName}`);
            ...
            logger.info(`An error occured in mongo when retrieving bucket for user: ${userName}`);
            return new Result<IDocumentUser>({
                success: false,
                data: null,
    ...
    static async createUser(user: IUser): Promise<Result<IDocumentUser>> {
        ...
        let existingUser = await UserModel.findOne({ username: user.userName }) as IDocumentUser;
            ...
            try {
                ...
                if (!(error instanceof Error)) {
                    ...
                    return new Result<IDocumentUser>({
                        success: false,
                        data: null,
                        error: new UserUnexpectedError('An unknown error occurred during user creation')
                    });
                }
