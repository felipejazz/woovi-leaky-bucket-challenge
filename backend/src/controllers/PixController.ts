...
export class PixController {
  static async simulatePixQuery(ctx: Context) {
    ...
    const makePixResult = await PixService.makePix({
      userName,
      token,
      key,
      value,
    ...
    ctx.body = {
      ...
      tokensLeft:
        tokenCount !== null && tokenCount !== undefined ? tokenCount : null,
  ...
  static handleError({
    ...
    ctx: Context;
    error: Error;
    userName: string | IDocumentUser;
    tokensLeft: number | null;
    newUserToken: string | null;
  }): void {
    switch (error.constructor) {
      case NoTokenToConsumeError || BucketNoValidTokensError:
        logger.warn(
          `Error during Pix query for user: ${userName}, error: ${error.message}`
        );
        ctx.status = 429;
        ctx.body = {
      ...
      case BucketRevokeTokenError:
        logger.error(
          `Error during Pix query for user: ${userName}, error: Token already revoked`
        );
        ctx.status = 400;
        ctx.body = {
      ...
      case InvalidPixKeyError:
        logger.error(
          `Error during Pix query for user: ${userName}, error: Invalid PIX key`
        );
        ctx.status = 400;
        ctx.body = {
          errorMessage: error.message,
      ...
      case InvalidPixValueError:
        logger.error(
          `Error during Pix query for user: ${userName}, error: Invalid PIX value`
        );
        ctx.status = 400;
        ctx.body = {
          errorMessage: error.message,
