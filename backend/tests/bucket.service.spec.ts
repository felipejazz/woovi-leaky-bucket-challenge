...
import {
  ...
  BucketTokenNotFoundError,
  NoTokenToConsumeError,
} from '../src/interfaces/Bucket/Errors';
...
describe('BucketService', () => {
  ...
  it('should create a bucket', async () => {
    const bucketCreateResult =
      await BucketService.createBucket(createdTestUser);
  ...
  it('should update the bucket', async () => {
    ...
    const bucketGetResult = await BucketService.getBucket(
      createdTestUser.userName
    );
  ...
  it('should add a token to the bucket', async () => {
    ...
    const bucketCountResult =
      await BucketService.getTokenCount(createdTestUser);
  ...
  it('should revoke a token', async () => {
    ...
    const revokeResult = await BucketService.revokeToken({
      bucket: bucket,
      token: token,
    });
    if (!revokeResult) {
    ...
    const userRevokedResult = await UserService.getUser(
      createdTestUser.userName
    );
  ...
  it('should not allow more than 10 tokens', async () => {
    ...
    for (let i = testBucket.tokens.length; i < 10; i++) {
      const token: string = UserService.generateToken(createdTestUser.userName);
      const additionResult = await BucketService.addToken({
        token: token,
        bucket: testBucket,
      });
      if (!additionResult) {
    ...
    const extraToken: string = UserService.generateToken(
      createdTestUser.userName
    );
    const addExtraTokenResult = await BucketService.addToken({
      token: extraToken,
      bucket: testBucket,
    });
  ...
  it('should add a token after 1 hour and 2 tokens after 2 hours', async () => {
    const initialBucketCreateResult =
      await BucketService.createBucket(createdTestUser);
    if (!initialBucketCreateResult.success) {
      throw initialBucketCreateResult.error;
    }

    const initialBucketGetResult = await BucketService.getBucket(
      createdTestUser.userName
    );
    ...
    const bucketGetResultAfter1Hour = await BucketService.getBucket(
      createdTestUser.userName
    );
    ...
    const updateBucketResultAfter2Hours =
      await BucketService.updateBucket(bucketAfter1Hour);
    if (!updateBucketResultAfter2Hours.success) {
      throw new Error('Error updating bucket during test');
    }

    const bucketGetResultAfter2Hours = await BucketService.getBucket(
      createdTestUser.userName
    );
    ...
    const bucketAfter2Hours =
      bucketGetResultAfter2Hours.data as IDocumentBucket;
